/* =========================================================
   ...
   Change — 1.2.2:
   - Updates the installed Gmail routing version assertion from 2.3.1 to 2.3.2
     after the approved batch-refresh quota safety release.
   - Preserves all Source Email cleanup and evidence-display regression checks.
   ========================================================= */
...
assert(source.includes("Version: 2.3.2"), "Today Gmail source display must identify version 2.3.2.");
assert(source.includes("function cleanSourceEmail(value)"), "Universal Source Email cleanup function is missing.");
assert(source.includes("function decodeEmailText(value)"), "Shared Gmail entity decoding function is missing.");
...
assert(source.includes("const source = formatConversationSource(message)"), "Morning Command must render the cleaned Source Conversation text.");
