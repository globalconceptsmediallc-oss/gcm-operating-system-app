/* =========================================================
   ...
   Purpose: Make Needs Action mean work that actually requires attention now,
            while keeping waiting and scheduled Media work visible separately.

   Production rules:
   - Read-only dashboard enhancement; no D1 writes occur here.
   - Durable Creative Work State is read from media_creative_history.
   - Waiting / Scheduled Creatives do not count as Needs Action until a saved
     resume date arrives.
   - Station confirmation waits remain visible but are not “do this now.”
   - Placement decisions reopen in the Media decision workspace.
   ========================================================= */
...
(() => {
  ...
  function placementDecisionHandled(record) {
    ...
    return (name === "retire_at_end" && lower(value.replacementRequired) === "no") || (name === "replacement_in_progress" && lower(value.replacementRequired) === "yes" && /^\d+$/.test(String(value.replacementCreativeId || "")));
  }
  ...
  function workStateFor(creative) {
    ...
    if (["waiting", "scheduled"].includes(explicit.state)) {
      const due = Boolean(explicit.resumeDate && explicit.resumeDate <= todayIso());
      return { state: due ? "active" : "waiting", due, waitingFor: explicit.waitingFor || "scheduled work", resumeDate: explicit.resumeDate };
    }
    if (lane === "awaiting") return { state: "waiting", due: false, waitingFor: "station confirmation", resumeDate: "" };
    return { state: "active", due: false, waitingFor: "", resumeDate: "" };
  }

  function creativeNeedsAction(creative) {
    const lane = creativeLane(creative);
    return ["preparing", "ready"].includes(lane) && workStateFor(creative).state !== "waiting";
  }
  ...
  function matchesPlacementFilters(record) {
    const clientId = Number($("client")?.value || 0), type = lower($("media-type-filter")?.value), status = lower($("status-filter")?.value), market = lower($("market-filter")?.value), outlet = lower($("outlet-filter")?.value), dateView = $("date-filter")?.value || "", search = lower($("search-filter")?.value);
    ...
    if (search) {
      const value = disposition(record);
      const haystack = [record.clientName, record.clientCode, record.campaignName, record.creativeName, record.fileName, record.market, record.outletName, record.status, record.trafficStatus, record.confirmationStatus, record.attentionReason, value.replacementCreativeId, record.mediaType].map(lower).join(" ");
  ...
  function matchesCreativeFilters(creative) {
    const clientId = Number($("client")?.value || 0), type = lower($("media-type-filter")?.value), status = lower($("status-filter")?.value), market = lower($("market-filter")?.value), outlet = lower($("outlet-filter")?.value), dateView = $("date-filter")?.value || "", search = lower($("search-filter")?.value);
    const assignments = assignmentsFor(creative.id), lane = creativeLane(creative), work = workStateFor(creative);
  ...
  function creativeReason(creative) {
    ...
    if (work.due) return `Scheduled work for Creative #${creative.id} is due now at ${stage}.`;
    if (lower(stage) === "working script") return `Creative #${creative.id} is at Working Script and is ready for the next approved copy.`;
    return `Creative #${creative.id} is at ${stage}.`;
  }

  function actionItems() {
    const placements = records.filter(matchesPlacementFilters).filter(record => placementUrgency(record) >= 70).map(record => ({ source: "placement", id: record.id, group: placementGroup(record), client: record.clientName || record.clientCode || "Client", title: record.campaignName || record.creativeName || "Media placement", market: [record.market, record.outletName].filter(Boolean).join(" / "), stage: "", reason: placementReason(record), deadline: record.stationDeadline ? `Traffic deadline ${dateText(record.stationDeadline)}` : "" }));
    const creatives = (workflow?.creatives || []).filter(matchesCreativeFilters).filter(creativeNeedsAction).map(creative => ({ source: "creative", id: creative.id, group: "Production / Preparation", client: creative.clientName || creative.clientCode || "Client", title: creative.creativeName || "Creative", market: assignmentsFor(creative.id).map(item => `${item.market || "Market"} / ${item.outletName || "Outlet"}`).join("; "), stage: creative.currentStage || "Idea / Direction", reason: creativeReason(creative), deadline: "" }));
    return [...placements, ...creatives];
  }

  function waitingItems() {
    const placements = records.filter(matchesPlacementFilters).filter(record => !isExpired(record) && isAwaiting(record) && !needsAttention(record)).map(record => ({ source: "placement", id: record.id, client: record.clientName || record.clientCode || "Client", title: record.campaignName || record.creativeName || "Media placement", market: [record.market, record.outletName].filter(Boolean).join(" / "), stage: "Awaiting Confirmation", reason: "Waiting for station/outlet confirmation.", deadline: "" }));
    const creatives = (workflow?.creatives || []).filter(matchesCreativeFilters).filter(creative => workStateFor(creative).state === "waiting" && !["running", "history"].includes(creativeLane(creative))).map(creative => {
      const work = workStateFor(creative);
      return { source: "creative", id: creative.id, client: creative.clientName || creative.clientCode || "Client", title: creative.creativeName || "Creative", market: assignmentsFor(creative.id).map(item => `${item.market || "Market"} / ${item.outletName || "Outlet"}`).join("; "), stage: creative.currentStage || "Idea / Direction", reason: `Waiting for ${work.waitingFor || "the next dependency"}${work.resumeDate ? ` · resumes ${dateText(work.resumeDate)}` : ""}.`, deadline: "" };
    });
    return [...placements, ...creatives];
  }

  function itemHtml(item, waiting = false) {
    const location = item.market ? `<span class="gcm-na-meta">${esc(item.market)}</span>` : "";
    const stage = item.stage ? `<span class="gcm-na-chip${waiting ? " gcm-na-wait-chip" : ""}">${esc(item.stage)}</span>` : "";
    const deadline = item.deadline ? `<span class="gcm-na-meta">${esc(item.deadline)}</span>` : "";
    const action = item.source === "creative" ? `<button type="button" class="gcm-na-open" data-open-creative="${esc(item.id)}">Open Production</button>` : `<button type="button" class="gcm-na-open" data-review-placement="${esc(item.id)}">Review</button>`;
  ...
  function groupHtml(name, items, waiting = false) {
    if (!items.length) return "";
    return `<section class="gcm-na-group${waiting ? " gcm-na-wait-group" : ""}"><div class="gcm-na-group-title"><strong>${esc(name)}</strong><span>${items.length}</span></div>${items.map(item => itemHtml(item, waiting)).join("")}</section>`;
  }

  function syncSummary(actionCount, waitingCount) {
    const count = $("count-action");
    if (count) count.textContent = String(actionCount);
    const detail = $("detail-action");
    if (detail) detail.textContent = actionCount ? `${actionCount} record${actionCount === 1 ? "" : "s"} require work now${waitingCount ? ` · ${waitingCount} waiting/scheduled` : ""}.` : `No immediate action${waitingCount ? ` · ${waitingCount} waiting/scheduled` : ""}.`;
  }
  ...
  function render() {
    ...
    syncSummary(items.length, waiting.length);
    const order = ["Decision Needed", "Production / Preparation", "Traffic / Placement Attention"];
    ...
    panel.innerHTML = `
      <div class="gcm-na-head"><div><strong>Media Work Detail</strong><p>What needs attention now, plus work that is waiting or scheduled.</p></div><span class="gcm-na-total">${items.length} action${items.length === 1 ? "" : "s"}</span></div>
      <div class="gcm-na-groups">${order.map(name => groupHtml(name, groups.get(name) || [])).join("")}${groupHtml("Waiting / Scheduled", waiting, true)}</div>
      ${items.length ? "" : '<div class="gcm-na-empty">No Media action is required right now.</div>'}`;
    panel.hidden = !expanded;
  }

  function mountPanel() {
    const countNode = $("count-action"), card = countNode?.closest("article"), summary = card?.parentElement;
    if (!card || !summary) return false;
    if (!$("gcm-needs-action-panel")) summary.insertAdjacentHTML("afterend", '<section id="gcm-needs-action-panel" class="gcm-na-panel" hidden aria-label="Media Work Detail"></section>');
    card.classList.add("gcm-na-summary-card");
    card.setAttribute("role", "button");
    card.setAttribute("tabindex", "0");
    card.setAttribute("aria-expanded", String(expanded));
    card.setAttribute("aria-controls", "gcm-needs-action-panel");
    card.title = "Show current Media actions and waiting work";
    return true;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .gcm-na-summary-card{cursor:pointer;transition:border-color .15s ease,transform .15s ease,box-shadow .15s ease}.gcm-na-summary-card:hover{border-color:#aac4eb;transform:translateY(-1px);box-shadow:0 10px 22px rgba(31,104,216,.1)}.gcm-na-summary-card:focus-visible{outline:3px solid rgba(31,104,216,.24);outline-offset:2px}.gcm-na-summary-card:after{content:"View work detail ↓";display:block;margin-top:10px;color:#1f68d8;font-size:.68rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em}.gcm-na-summary-card[aria-expanded="true"]:after{content:"Hide work detail ↑"}
      .gcm-na-panel{margin:-4px 0 18px;padding:18px;border:1px solid #b9cfee;border-radius:16px;background:#fff;box-shadow:0 8px 24px rgba(7,20,38,.07)}.gcm-na-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding-bottom:14px;border-bottom:1px solid #dbe2ec}.gcm-na-head strong{color:#071426;font-size:1rem}.gcm-na-head p{margin:4px 0 0;color:#637083;font-size:.78rem}.gcm-na-total{display:inline-flex;align-items:center;min-height:30px;padding:0 10px;border-radius:999px;background:#eaf2ff;color:#1f68d8;font-size:.72rem;font-weight:900}.gcm-na-groups{display:grid;gap:14px;margin-top:14px}.gcm-na-group{border:1px solid #dbe2ec;border-radius:13px;overflow:hidden}.gcm-na-wait-group{border-color:#e1d3ae}.gcm-na-group-title{display:flex;justify-content:space-between;align-items:center;padding:11px 13px;background:#f5f7fb;color:#071426}.gcm-na-wait-group .gcm-na-group-title{background:#fff8e8}.gcm-na-group-title strong{font-size:.82rem}.gcm-na-group-title span{display:grid;place-items:center;min-width:24px;height:24px;padding:0 7px;border-radius:999px;background:#eaf2ff;color:#1f68d8;font-size:.68rem;font-weight:900}.gcm-na-item{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:center;padding:13px;border-top:1px solid #dbe2ec}.gcm-na-heading{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.gcm-na-heading strong{color:#071426;font-size:.82rem}.gcm-na-chip{display:inline-flex;align-items:center;min-height:23px;padding:0 8px;border-radius:999px;background:#fff4dd;color:#8a5b12;font-size:.62rem;font-weight:900;text-transform:uppercase}.gcm-na-wait-chip{background:#edf5ff;color:#245fae}.gcm-na-sub{display:flex;gap:10px;flex-wrap:wrap;margin-top:4px}.gcm-na-meta{color:#637083;font-size:.7rem;font-weight:750}.gcm-na-item p{margin:6px 0 0;color:#34445b;font-size:.76rem}.gcm-na-open{min-height:36px;padding:0 11px;border:1px solid #c5d2e4;border-radius:9px;background:#fff;color:#0b1d33;font:inherit;font-size:.72rem;font-weight:900;cursor:pointer}.gcm-na-open:hover{border-color:#8fb4eb;background:#edf5ff}.gcm-na-empty{padding:18px 0 2px;color:#637083;font-size:.8rem}@media(max-width:760px){.gcm-na-item{grid-template-columns:1fr}.gcm-na-open{justify-self:start}}
    `;
  ...
  async function fetchData() {
    const [placementPayload, creativePayload] = await Promise.all([post({ action: "get-media-operations" }), post({ action: "get-media-operations", operation: "get_creative_workflow" })]);
  ...
  function togglePanel(force) {
    ...
    const card = $("count-action")?.closest("article"), panel = $("gcm-needs-action-panel");
    if (card) card.setAttribute("aria-expanded", String(expanded));
    if (panel) panel.hidden = !expanded;
    if (expanded) { render(); panel?.scrollIntoView({ behavior: "smooth", block: "nearest" }); }
  }
  ...
  function start() {
    ...
    document.addEventListener("click", event => {
      ...
      if (summaryCard) { event.preventDefault(); togglePanel(); return; }
      const placementButton = event.target.closest("[data-review-placement]");
      if (placementButton) { event.preventDefault(); reviewPlacement(placementButton.dataset.reviewPlacement); return; }
      const creativeButton = event.target.closest("[data-open-creative]");
      if (creativeButton?.closest("#gcm-needs-action-panel")) { event.preventDefault(); openCreative(creativeButton.dataset.openCreative); return; }
      if (event.target?.id === "refresh") setTimeout(refreshData, 500);
      if (event.target?.id === "clear-filters") scheduleRender(600);
    }, true);
    document.addEventListener("keydown", event => {
      if (event.target?.classList?.contains("gcm-na-summary-card") && ["Enter", " "].includes(event.key)) { event.preventDefault(); togglePanel(); }
    });
    ...
    document.addEventListener("input", event => { if (event.target?.id === "search-filter") scheduleRender(300); }, true);
    window.addEventListener("gcm-media-work-state-changed", () => setTimeout(refreshData, 250));
  }
