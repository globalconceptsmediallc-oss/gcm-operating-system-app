/* =========================================================
   ...
   Source: gcm-shell.js 2.0.9 production navigation
   Sprint: Media Placement End-of-Run Decision
   Change: Loads Media placement disposition v1.0.0 while preserving all
           existing Media creative, recovery, and market Traffic ID enhancements.
   ========================================================= */

(() => {
  "use strict";

  const SHELL_VERSION = "2.0.10";
  ...
  const DEFERRED_NAV = [{ label: "Settings", icon: "⚙", reason: "Workflow not yet active" }];
  ...
  function navItemHtml(key,item,currentPage) {
    const currentAttribute = key === currentPage ? ' aria-current="page"' : "";
    return `<a class="gcm-shell-nav-link" href="${escapeHtml(item.href)}" data-gcm-nav="${escapeHtml(key)}"${currentAttribute}><span class="gcm-shell-nav-icon" aria-hidden="true">${escapeHtml(item.icon)}</span><span>${escapeHtml(item.label)}</span></a>`;
  }
  ...
  function buildSidebarHtml(currentPage,statusText) {
    const activePage = Object.prototype.hasOwnProperty.call(PAGE_MAP,currentPage) ? currentPage : "";
    const primaryNavigation = Object.entries(PAGE_MAP).map(([key,item])=>navItemHtml(key,item,activePage)).join("");
    const deferredNavigation = DEFERRED_NAV.map(deferredItemHtml).join("");
  ...
  function configureMobileMenu(sidebar,menuButton) {
    ...
    menuButton.setAttribute("aria-expanded",String(sidebar.classList.contains("is-open")));
    menuButton.addEventListener("click",()=>{const isOpen=sidebar.classList.toggle("is-open");menuButton.setAttribute("aria-expanded",String(isOpen));});
    document.addEventListener("click",event=>{if(window.innerWidth>860||!sidebar.classList.contains("is-open")||sidebar.contains(event.target)||menuButton.contains(event.target))return;sidebar.classList.remove("is-open");menuButton.setAttribute("aria-expanded","false");});
    sidebar.addEventListener("click",event=>{const navLink=event.target.closest(".gcm-shell-nav-link");if(!navLink||window.innerWidth>860)return;sidebar.classList.remove("is-open");menuButton.setAttribute("aria-expanded","false");});
  }
  ...
  function loadPageEnhancements() {
    ...
    if(/\/media\.html$/i.test(path)) {
      ...
      appendScript("shared/media-placement-disposition.js?v=1.0.0","data-gcm-media-placement-disposition");
    }
  }
  ...
  function mount(options={}) {
    ...
    const mobileMenuSelector=options.mobileMenuSelector||"[data-gcm-shell-menu-button]";
    const sidebar=document.querySelector(sidebarSelector);
    if(!sidebar){console.error(`GCM OS Shell ${SHELL_VERSION}: sidebar mount point not found: ${sidebarSelector}`);return false;}
    const currentPage=String(options.currentPage||"").trim().toLowerCase();
    const statusText=String(options.statusText||"GCM OS operational").trim()||"GCM OS operational";
    if(!sidebar.id)sidebar.id="gcm-os-sidebar";
    sidebar.classList.add("gcm-shell-sidebar");
    sidebar.innerHTML=buildSidebarHtml(currentPage,statusText);
    configureMobileMenu(sidebar,document.querySelector(mobileMenuSelector));
    loadPageEnhancements();
    return true;
  }

  window.GCMOShell=Object.freeze({version:SHELL_VERSION,mount});
})();
