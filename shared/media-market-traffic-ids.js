...
(() => {
  ...
  function creative() {
    ...
    return workflow?.creatives?.find(item => Number(item.id) === creativeId) || null;
  }

  function assignmentFor(market, outletName) {
    const creativeId = currentCreativeId();
    return workflow?.assignments?.find(item =>
      Number(item.creativeId) === creativeId &&
  ...
  function updateVersionAndCreativeLabel() {
    ...
    const globalInput = $("isci");
    const label = globalInput?.closest("label");
    if (label) {
  ...
  function decoratePackageMarkets() {
    document.querySelectorAll("#pkgMarkets .pkgmarket").forEach(label => {
      label.querySelector(".market-traffic-id-display")?.remove();
      ...
      const trafficId = trafficIdForAssignment(assignment) || (document.querySelectorAll("#pkgMarkets .pa:checked").length === 1 ? clean(creative()?.isci) : "");
      if (!trafficId) return;
  ...
  async function saveMarketState(requireComplete = false) {
    ...
    const trafficAssignments = rows
      .filter(row => clean(row.trafficId))
      .map(row => ({ market: row.market, outletName: row.outletName, trafficId: row.trafficId }));

    if (trafficAssignments.length) {
      await api("save_assignment_traffic_ids", { creativeId, assignments: trafficAssignments });
      await refreshData();
    }
  ...
  async function continueToStationEmail() {
    ...
    await api("save_creative", {
      creativeId,
      creative: creativePayload("Station Email Package")
    });
  ...
  function selectedAssignments() {
    const creativeId = currentCreativeId();
    const ids = [...document.querySelectorAll("#pkgMarkets .pa:checked")]
  ...
  function assignmentTrafficId(assignment, selectedCount) {
    const marketId = trafficIdForAssignment(assignment);
    if (marketId) return marketId;
    if (selectedCount === 1) return clean(creative()?.isci);
    return "";
  }
  ...
  async function buildStationEmail() {
    ...
    const rows = assignments.map(assignment => ({
      ...assignment,
      trafficId: assignmentTrafficId(assignment, assignments.length)
    }));

    for (const row of rows) {
      if (!row.rotationStartDate || !row.rotationEndDate) {
        throw new Error(`${row.market} / ${row.outletName} does not have a Commercial Schedule yet.`);
      }
      if (!row.trafficId) {
        throw new Error(`${row.market} / ${row.outletName} does not have a Traffic ID / ISCI yet. Return to Markets and save it first.`);
      }
    }
    ...
    const scriptLabel = scriptSource === "none"
      ? ""
      : scriptSource === "coop_script"
        ? "co-op script"
        : scriptSource === "approved_script"
          ? "approved script"
          : "final script";
    ...
    if (clean($("special")?.value)) {
      body.push("", "Special Instructions:", "", clean($("special").value));
    }
    ...
    body.push(
      ...
      scriptSource === "none"
        ...
        : `The final production audio and ${scriptLabel} are attached.`,
  ...
  function replaceButton(node, handler, statusTarget) {
    ...
    replacement.addEventListener("click", async () => {
      ...
      try {
        ...
        setStatus(statusTarget, error.message || String(error), "err");
      } finally {
  ...
  function observeDynamicAreas() {
    ...
    if (marketList && !marketObserver) {
      marketObserver = new MutationObserver(() => injectTrafficInputs());
      marketObserver.observe(marketList, { childList: true, subtree: true });
    }

    const packageMarkets = $("pkgMarkets");
    if (packageMarkets && !packageObserver) {
      packageObserver = new MutationObserver(() => decoratePackageMarkets());
      packageObserver.observe(packageMarkets, { childList: true, subtree: true });
    }
  }
  ...
  async function start() {
    try {
      ...
      document.querySelectorAll(".step").forEach(step => {
        step.addEventListener("click", () => setTimeout(async () => {
          try {
            await refreshData();
            injectTrafficInputs();
            decoratePackageMarkets();
