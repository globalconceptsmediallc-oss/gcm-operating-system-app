/* =========================================================
   ...
   Source: Production routes/clientWorkspace.js 7.4.0
   Sprint: Client Workspace — Targeted Work Deep Links
   ...
   Production changes — 7.4.1:
   - Adds the authoritative D1 client_code to Investigation and Work Item links
     generated by the client workspace.
   - Makes Next Best Action, Business Priority, and Current Work handoffs target
     the correct client before opening the requested Work record.
   - Preserves Client Health v2, live D1 records, monitoring boundaries, and
     all existing workspace data contracts.

   Previous production changes — 7.4.0:
  ...
  return {
    ...
    workspace: {
      ...
      nextActionHref: firstInvestigation
        ? `work.html?investigation=${encodeURIComponent(firstInvestigation.id)}&client=${encodeURIComponent(client.client_code)}`
        : firstWork
          ? `work.html?workItem=${encodeURIComponent(firstWork.id)}&client=${encodeURIComponent(client.client_code)}`
          : null,
      nextReviewDate:
        dateOnly(
    ...
    currentPriorities: [
      ...openInvestigations.map(item =>
        mapInvestigationToPriority(item, client.client_code)
      ),
      ...activeAlerts.map(mapAlertToPriority)
    ],

    workQueue: openWork.map(item =>
      mapWorkItem(item, client.client_code)
    ),
...
function mapInvestigationToPriority(item, clientCode) {
  return {
    ...
    href: `work.html?investigation=${encodeURIComponent(item.id)}&client=${encodeURIComponent(clientCode)}`,
...
function mapWorkItem(item, clientCode) {
  return {
    id: `work-${item.id}`,
    title: item.title,
    ...
    proofOfWorkId: null,
    href: `work.html?workItem=${encodeURIComponent(item.id)}&client=${encodeURIComponent(clientCode)}`
  };
}
