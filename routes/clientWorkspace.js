<!-- =========================================================
   ...
   Source: Production work.html 1.9.0
   Sprint: Client Tool Router Response Compatibility
   Purpose: Preserve the guided evidence workflow while reading authoritative
            client tools from every supported Worker response location and
            exposing a visible diagnostic during production road testing.

   Production changes:
   - Adds Capture Evidence Screenshot inside Guided Investigation.
   - Uses the existing analyze-client-communication Workers AI pipeline.
   - Displays extracted evidence before any operational decision is saved.
   - Prefills Investigation Finding, Specific Work Required, and Specific Action.
   - Human approval remains required before production processing.
   - Adds a one-time client-specific SEMrush project URL setup.
   - Saves the URL locally by client code without changing D1.
   - Open SEMrush Site Audit uses the saved direct project URL when available.
   - Adds a live Evidence Workflow checklist to the Guided Investigation.
   - Checklist advances when SEMrush is opened, a screenshot is chosen,
     Workers AI completes analysis, findings are reviewed, and work is created.
   - Checklist state is stored locally by client and Investigation for road testing.
   - Removes the manual SEMrush URL text-entry field from the Investigation.
   - Adds Save Copied Issues Link for one-time client setup.
   - Saved links change the action to Open <Client> Issues.
   - Removes browser-local SEMrush setup from the Investigation.
   - Uses client_tools returned by get-client-workspace.
   - Routes Site Audit, Position Tracking, Backlink Audit, Search Console,
     website, Cloudflare, GitHub, and future tools by verified tool_key.
   - Reads tools from operational.clientTools, top-level clientTools,
     clientToolRouter, and record.tools.
   - Adds a visible router diagnostic showing the resolved tool key and count.

   Supported URLs:
   - work.html?investigation=36&client=PW
   - work.html?workItem=4&client=SFS
   ========================================================= -->
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;min-height:100vh;background:var(--background);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .router-diagnostic{margin-top:9px;padding:9px 10px;border:1px solid #d7e2f0;border-radius:8px;background:#f7f9fc;color:var(--muted);font-size:.68rem;font-weight:750;line-height:1.4}
    .router-diagnostic.ready{border-color:#b9dfc8;background:var(--green-soft);color:var(--green)}
    .router-diagnostic.missing{border-color:#efcccc;background:var(--red-soft);color:var(--red)}
    ...
    .tool-link-input-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}
    .tool-link-input-row input{min-height:40px}
    ...
    @media(max-width:700px){.tool-link-input-row{grid-template-columns:1fr}.evidence-step{grid-template-columns:28px minmax(0,1fr)}.evidence-step-status{grid-column:2}.evidence-workflow-header{flex-direction:column}}


.processing-actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:12px}.work-card{padding:16px;border:1px solid var(--border);border-radius:10px;background:#fff;margin-bottom:12px}.target-banner{margin-bottom:16px;padding:13px;border:1px solid #aac7ed;border-radius:10px;background:var(--blue-soft);color:#205c9f;font-size:.82rem;font-weight:800}
    @media(max-width:1100px){.workspace-grid{grid-template-columns:1fr}.investigation-list{max-height:none}}@media(max-width:860px){.app-shell{display:block}main{padding:18px 14px 32px}.summary-grid{grid-template-columns:1fr}}
  </style>
</head>
<body>
<div class="app-shell">
  ...
  <main>
    <div class="page">
      <header class="page-header">
        <div><p class="eyebrow">Mission Control</p><h1>Work Queue</h1><p class="subtitle">Investigate one evidence step at a time. Create specific work only after the evidence proves what must be done.</p></div>
        <span class="classification">Client Tool Router Diagnostic · v1.9.1</span>
      </header>
      ...
      <section id="open-work-panel" class="panel"><div class="panel-header"><h2>Open Work Items</h2><p>Confirmed specific work created from Investigation findings.</p></div><div id="open-work-list" class="panel-body"></div></section>
...
(() => {
  ...
  if(window.GCMOShell){window.GCMOShell.mount({currentPage:"work",statusText:"Client tool router diagnostic · Work v1.9.1"});}
  const el={openInvestigations:byId("open-investigations"),clients:byId("clients-with-investigations"),workItems:byId("work-items"),filter:byId("client-filter"),refresh:byId("refresh-button"),status:byId("runtime-status"),list:byId("investigation-list"),detail:byId("detail-panel"),openWork:byId("open-work-list"),target:byId("target-banner")};
  let activeClients=[],investigations=[],workItems=[],selectedKey="",processing=false,processingWorkItem=false;
  ...
  async function load(){
    ...
    try{
      activeClients=await loadAttentionClients();
      ...
      results.forEach((result,index)=>{
        ...
        const clientTools=collectClientTools(result.value);
        const clientToolRouter=collectClientToolRouter(result.value,clientTools);
        (Array.isArray(op.investigations)?op.investigations:[]).filter(i=>!closed(i)).forEach(i=>investigations.push({...i,clientCode:op.client?.client_code||client.code,clientName:op.client?.name||client.name,clientWebsite,clientTools,clientToolRouter,communication:communicationById.get(Number(i.communication_id))||null}));
        (Array.isArray(op.workItems)?op.workItems:[]).filter(i=>!completed(i)).forEach(i=>workItems.push({...i,clientCode:op.client?.client_code||client.code,clientName:op.client?.name||client.name,clientWebsite,clientTools,clientToolRouter}));
      });
      investigations.sort(sortInvestigations);
      renderSummary();renderWorkItems();applyRequestedTarget();
  ...
  async function post(body){const response=await fetch(WORKER_URL,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(body)});const payload=await response.json().catch(()=>({}));if(!response.ok||payload.ok!==true)throw new Error(payload.error||payload.details||`HTTP ${response.status}`);return payload;}
  ...
  function selectVisibleDefault(){const rows=visible();if(!rows.length){selectedKey="";renderList();el.detail.innerHTML='<div class="detail-empty">No open Investigations match this view.</div>';return;}selectInvestigation(rows[0]);}
  function applyRequestedTarget(){
    renderList();
    if(requestedWorkItemId){
      const item=workItems.find(i=>Number(i.id)===requestedWorkItemId);
      if(item){showTarget(`Opened Work Item #${item.id} for ${item.clientName}.`);document.querySelector(`[data-work-id="${item.id}"]`)?.scrollIntoView({behavior:"smooth",block:"center"});return;}
    }
  ...
  function selectInvestigation(item,scroll=false){if(!item)return;selectedKey=key(item);renderList();renderDetail(item);if(scroll)byId("guided-investigation-panel").scrollIntoView({behavior:"smooth",block:"start"});loadGuidance(item);}
  ...
  function investigationFamily(item){
    return norm(
      item?.communication?.communication_family ||
      item?.communication?.family ||
      item?.communication?.source ||
      item?.communication?.sender ||
      item?.title
    );
  }
  ...
  const EVIDENCE_STEPS=[
    {id:"open_semrush",title:"Open Client Site Audit",detail:"Open the correct client project or saved Issues page."},
  ...
  function bindEvidenceWorkflow(item){renderEvidenceWorkflowState(item);byId("reset-evidence-workflow")?.addEventListener("click",()=>{if(window.confirm("Reset the Evidence Workflow checklist for this Investigation?"))resetEvidenceWorkflow(item);});document.querySelector('[data-evidence-tool="primary_evidence"]')?.addEventListener("click",()=>completeEvidenceStep(item,"open_semrush"));const finding=byId("finding-summary"),title=byId("work-title"),description=byId("work-description");[finding,title,description].forEach(field=>field?.addEventListener("input",()=>{if(String(finding?.value||"").trim()&&String(title?.value||"").trim()&&String(description?.value||"").trim())completeEvidenceStep(item,"review_findings");}));}

  function collectClientTools(payload){
    const operational=payload?.operational||{};
    const record=payload?.record||{};

    const candidates=[
      operational.clientTools,
      payload?.clientTools,
      record.tools
    ];
    ...
    for(const candidate of candidates){
      if(Array.isArray(candidate)&&candidate.length){
        return candidate.map(tool=>({
          id:tool.id,
          client_id:tool.client_id||tool.clientId,
          tool_family:tool.tool_family||tool.family,
          tool_key:tool.tool_key||tool.key,
          label:tool.label,
          url:tool.url,
          status:String(tool.status||"active").toLowerCase(),
          notes:tool.notes||""
        }));
      }
    }

    const router=payload?.clientToolRouter||operational.clientToolRouter;
    if(router&&typeof router==="object"){
      return Object.values(router).map(tool=>({
        id:tool.id,
        client_id:tool.client_id||tool.clientId,
        tool_family:tool.tool_family||tool.family,
        tool_key:tool.tool_key||tool.key,
  ...
  function collectClientToolRouter(payload,tools){
    const operational=payload?.operational||{};
    const supplied=payload?.clientToolRouter||operational.clientToolRouter;

    if(supplied&&typeof supplied==="object"){
      return supplied;
    }

    return tools.reduce((router,tool)=>{
      const key=normalizeToolKey(tool.tool_key||tool.key);
      if(key&&tool.url){
        router[key]=tool;
      }
      return router;
  ...
  const TOOL_KEYS={
    SEMRUSH_SITE_AUDIT_ISSUES:"semrush_site_audit_issues",
    SEMRUSH_SITE_AUDIT_OVERVIEW:"semrush_site_audit_overview",
    SEMRUSH_POSITION_TRACKING:"semrush_position_tracking",
    SEMRUSH_BACKLINK_AUDIT:"semrush_backlink_audit",
    GOOGLE_SEARCH_CONSOLE:"google_search_console",
    GOOGLE_ANALYTICS:"google_analytics",
    GOOGLE_BUSINESS_PROFILE:"google_business_profile",
    SHOPIFY_ADMIN:"shopify_admin",
    CLOUDFLARE_DASHBOARD:"cloudflare_dashboard",
  ...
  function clientToolMap(item){
    const map=new Map();

    if(item?.clientToolRouter&&typeof item.clientToolRouter==="object"){
      Object.entries(item.clientToolRouter).forEach(([rawKey,tool])=>{
        const key=normalizeToolKey(rawKey||tool?.tool_key||tool?.key);
        if(key&&tool?.url){
          map.set(key,{
            key,
            family:String(tool?.tool_family||tool?.family||"").trim(),
    ...
    (Array.isArray(item?.clientTools)?item.clientTools:[])
      .filter(tool=>String(tool?.status||"active").toLowerCase()==="active")
      .forEach(tool=>{
        const key=normalizeToolKey(tool?.tool_key||tool?.key);
        if(key&&tool?.url&&!map.has(key)){
          map.set(key,{
            key,
            family:String(tool?.tool_family||tool?.family||"").trim(),
            label:String(tool?.label||"").trim(),
  ...
  function investigationText(item){
    return [
      item?.title,
      item?.description,
      item?.recommendation,
  ...
  function preferredToolKeys(item){
    const text=investigationText(item);

    if(text.includes("site audit")){
      return [
        TOOL_KEYS.SEMRUSH_SITE_AUDIT_ISSUES,
        TOOL_KEYS.SEMRUSH_SITE_AUDIT_OVERVIEW
      ];
    }

    if(text.includes("position tracking")||text.includes("ranking")){
      return [TOOL_KEYS.SEMRUSH_POSITION_TRACKING];
    }

    if(text.includes("backlink")){
      return [TOOL_KEYS.SEMRUSH_BACKLINK_AUDIT];
    }

    if(text.includes("search console")||text.includes("gsc")){
      return [TOOL_KEYS.GOOGLE_SEARCH_CONSOLE];
    }

    if(text.includes("analytics")||text.includes("ga4")){
      return [TOOL_KEYS.GOOGLE_ANALYTICS];
    }

    if(text.includes("business profile")||text.includes("gbp")){
      return [TOOL_KEYS.GOOGLE_BUSINESS_PROFILE];
    }

    if(text.includes("shopify")){
      return [TOOL_KEYS.SHOPIFY_ADMIN];
    }

    if(text.includes("cloudflare")){
      return [TOOL_KEYS.CLOUDFLARE_DASHBOARD];
    }

    if(text.includes("github")||text.includes("repository")){
      return [TOOL_KEYS.GITHUB_REPOSITORY];
    }

    return [];
  }

  function resolveClientTool(item){
    const tools=clientToolMap(item);

    for(const key of preferredToolKeys(item)){
      const tool=tools.get(key);
      if(tool?.url){
        return {
          configured:true,
          key,
          label:tool.label||toolActionLabel(item,key),
          url:tool.url
        };
      }
    }
    ...
    return {
      configured:false,
      key:preferredToolKeys(item)[0]||"",
      label:"Tool Setup Required",
      url:`business-workspace.html?business=${encodeURIComponent(item.clientCode)}`
    };
  }

  function toolActionLabel(item,key){
    const client=item.clientName||"Client";
    const labels={
      [TOOL_KEYS.SEMRUSH_SITE_AUDIT_ISSUES]:`Open ${client} Issues`,
      [TOOL_KEYS.SEMRUSH_SITE_AUDIT_OVERVIEW]:`Open ${client} Site Audit`,
      [TOOL_KEYS.SEMRUSH_POSITION_TRACKING]:`Open ${client} Position Tracking`,
      [TOOL_KEYS.SEMRUSH_BACKLINK_AUDIT]:`Open ${client} Backlink Audit`,
      [TOOL_KEYS.GOOGLE_SEARCH_CONSOLE]:`Open ${client} Search Console`,
      [TOOL_KEYS.GOOGLE_ANALYTICS]:`Open ${client} Analytics`,
      [TOOL_KEYS.GOOGLE_BUSINESS_PROFILE]:`Open ${client} Business Profile`,
      [TOOL_KEYS.SHOPIFY_ADMIN]:`Open ${client} Shopify`,
      [TOOL_KEYS.CLOUDFLARE_DASHBOARD]:`Open ${client} Cloudflare`,
      [TOOL_KEYS.GITHUB_REPOSITORY]:`Open ${client} Repository`
    };
    return labels[key]||`Open ${client} Evidence Tool`;
  }

  function buildEvidenceActions(item){
    const actions=[];
    const resolved=resolveClientTool(item);

    if(preferredToolKeys(item).length){
      actions.push({
        label:resolved.configured
          ? toolActionLabel(item,resolved.key)
          : resolved.label,
        href:resolved.url,
        primary:true,
        tool:"primary_evidence",
        internal:!resolved.configured
      });
    }
  ...
  function buildRouterDiagnostic(item){
    const tools=clientToolMap(item);
    const preferred=preferredToolKeys(item);
    const resolved=resolveClientTool(item);
    const available=[...tools.keys()];

    if(resolved.configured){
      return `<div class="router-diagnostic ready">Router ready · ${esc(item.clientCode)} · ${esc(resolved.key)} · ${available.length} active tool${available.length===1?"":"s"}</div>`;
    }

    return `<div class="router-diagnostic missing">Router missing · ${esc(item.clientCode)} · expected ${esc(preferred[0]||"no matching tool key")} · received ${available.length?esc(available.join(", ")):"0 tools"}</div>`;
  }
  ...
  function renderDetail(item,guidance=null){
    ...
    <div class="guided-box">
      ...
      <div class="evidence-actions">
        ...
        ${buildRouterDiagnostic(item)}
      </div>
      <div class="evidence-capture">
        <h4>Capture Evidence Screenshot</h4>
        <p>Upload the current SEMrush screen. The OS will extract visible evidence and prefill the Investigation fields for your review.</p>
        <div class="evidence-upload-row">
          ...
          <button class="button primary" id="analyze-investigation-evidence" disabled>Analyze Evidence</button>
        </div>
    ...
    <section class="detail-section"><h3>Final Resolution</h3><div class="processing-box"><label>Investigation Finding</label><textarea id="finding-summary">${esc(item.finding_summary||"")}</textarea><label>Specific Work Required</label><textarea id="work-title"></textarea><label>Specific Action</label><textarea id="work-description"></textarea><div class="processing-actions"><button id="close-button" class="button">Close — No Work Required</button><button id="create-button" class="button primary">Create Specific Work</button></div><div id="process-message"></div></div></section>`;
  ...
  function bindEvidenceCapture(item){
    ...
    analyze.addEventListener("click",async()=>{
      ...
      status.textContent="Workers AI is reading the screenshot and extracting evidence…";

      try{
        const context=[
          `Active client: ${item.clientName}`,
          `Client code: ${item.clientCode}`,
          `Investigation #${item.id}: ${item.title||"Investigation"}`,
          `Investigation objective: ${item.description||""}`,
          "This screenshot is investigation evidence, not a new inbound communication.",
          "Extract visible facts, metrics, issue names, issue counts, affected-page counts, and platform explanations."
        ].join("\n");
        ...
        prefillInvestigationFields(item,captured);

        completeEvidenceStep(item,"review_issues");
        completeEvidenceStep(item,"analyze_evidence");
        status.className="status ready capture-status";
        status.textContent="Evidence extracted. Review the captured facts and the prefilled Investigation fields before saving.";
      }catch(error){
        console.error(error);
        status.className="status error capture-status";
  ...
  function normalizeCapturedEvidence(payload){
    ...
    const facts=Array.isArray(evidence.visibleFacts)?evidence.visibleFacts.filter(Boolean):[];
    const metrics=Array.isArray(evidence.visibleMetrics)?evidence.visibleMetrics.filter(Boolean):[];
    const summary=String(
      analysis.operationalSummary ||
      analysis.summary ||
      payload?.consultantSummary?.summary ||
      ""
    ).trim();

    const combined=[...facts,...metrics];
    const unique=[...new Set(combined.map(value=>String(value).trim()).filter(Boolean))];

    return {
      facts:unique,
      summary,
      source:String(evidence.visibleSource||analysis.source||"Screenshot evidence").trim(),
      subject:String(evidence.visibleSubject||analysis.title||"Investigation evidence").trim(),
      confidence:String(evidence.confidence||analysis.classificationConfidence||"").trim(),
      recommendation:String(
        analysis.recommendedAction ||
        payload?.businessMeaning?.recommendedAction ||
        ""
      ).trim()
    };
  }

  function renderCapturedEvidence(captured){
    ...
    list.innerHTML=items.length
      ...
      : "<li>No structured evidence was extracted. Review the screenshot manually.</li>";

    panel.hidden=false;
  }

  function prefillInvestigationFields(item,captured){
    const finding=byId("finding-summary");
    const workTitle=byId("work-title");
    const workDescription=byId("work-description");

    const issueFacts=captured.facts.filter(value=>
      /error|warning|issue|page|url|sitemap|canonical|redirect|4xx|broken|meta description|javascript|css/i.test(value)
    );

    const evidenceText=(issueFacts.length?issueFacts:captured.facts).slice(0,12).join("; ");
    const findingText=[
      captured.summary,
      evidenceText&&`Captured evidence: ${evidenceText}`
    ].filter(Boolean).join("\n\n");

    if(!finding.value.trim()){
      finding.value=findingText||"Screenshot evidence was captured. Review the extracted facts and record the supported finding.";
    }

    if(!workTitle.value.trim()){
      workTitle.value=buildSpecificWorkTitle(item,captured);
    }

    if(!workDescription.value.trim()){
      workDescription.value=buildSpecificAction(item,captured);
    }
  }

  function buildSpecificWorkTitle(item,captured){
    const facts=captured.facts.join(" ").toLowerCase();

    if(facts.includes("sitemap")){
      return "Review and correct the verified sitemap.xml issues identified in the SEMrush Site Audit.";
    }
    if(facts.includes("4xx")){
      return "Review and correct the verified 4XX pages identified in the SEMrush Site Audit.";
    }
    if(facts.includes("canonical")){
      return "Review and correct the verified canonical URL issues identified in the SEMrush Site Audit.";
    }
    if(facts.includes("broken internal link")||facts.includes("internal link is broken")){
      return "Review and correct the verified broken internal link identified in the SEMrush Site Audit.";
    }

    return `Review and correct the highest-priority verified issue from Investigation #${item.id}.`;
  }

  function buildSpecificAction(item,captured){
    const facts=captured.facts.join(" ").toLowerCase();

    if(facts.includes("sitemap")){
      return "Open the SEMrush sitemap.xml issue details, capture the affected URLs and technical explanation, verify each listed URL against the live sitemap and website, correct only the evidence-supported sitemap conditions, deploy the changes, rerun the Site Audit, and record the verified result.";
    }
    if(facts.includes("4xx")){
      return "Open the SEMrush 4XX issue details, capture each affected URL and its referring source, verify the condition on the live website, repair or redirect the evidence-supported broken pages, rerun the Site Audit, and record the verified result.";
    }
    if(facts.includes("canonical")){
      return "Open the SEMrush canonical issue details, capture the affected URLs and technical explanation, verify the canonical implementation on the live pages, correct only the evidence-supported canonical conditions, rerun the Site Audit, and record the verified result.";
    }

    return "Open the selected SEMrush issue details, capture the affected URLs, issue count, technical explanation, and root cause; verify the condition on the live website; complete only the evidence-supported corrective work; rerun the Site Audit; and record the verified result.";
  }
  ...
  async function processInvestigation(item,outcome){
    ...
    if(outcome==="specific_work_required"&&(!title||!description)){message.className="status error";message.textContent="Specific Work Required and Specific Action are required.";return;}
  ...
  function renderWorkItems(){
    if(!workItems.length){
      el.openWork.innerHTML='<div class="empty">No open Work Items were returned.</div>';
      return;
    }

    el.openWork.innerHTML=workItems.map(i=>`
      <article class="work-card" data-work-id="${esc(i.id)}">
        <h3>Work Item #${esc(i.id)} · ${esc(i.title||"Work Item")}</h3>
        <p class="detail-subtitle">${esc(i.clientName)} · Investigation #${esc(i.investigation_id||"—")}</p>
        ...
        <section class="detail-section">
          ...
          <div class="processing-actions">
            <button class="button primary" data-complete-work="${esc(i.id)}">Complete Work Item</button>
          </div>
    ...
    el.openWork.querySelectorAll("[data-complete-work]").forEach(button=>{
      button.addEventListener("click",()=>{
        ...
        const item=workItems.find(entry=>Number(entry.id)===id);
  ...
  async function completeWorkItem(item){
    ...
    if(!window.confirm(`Complete Work Item #${id} and close its linked Investigation?`))return;
    ...
    try{
      await post({
        ...
        actualImpact,
        evidenceDescription,
        evidenceSource:"Guided Investigation Completion Evidence",
        evidenceType:"completion"
      });

      message.className="status ready";
      message.textContent=`Work Item #${id} completed and its linked Investigation closed.`;
      await load();
    }catch(error){
      message.className="status error";
